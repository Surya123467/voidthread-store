...
begin
  ...
  -- Lock every requested variant row before stock validation so concurrent checkouts cannot oversell.
  ...
  for item in select * from jsonb_array_elements(p_lines) loop
    ...
    -- COD is committed inventory immediately. Prepaid inventory is decremented on verified payment.
...
-- Product image bucket. Keep writes restricted to server/admin flows.
